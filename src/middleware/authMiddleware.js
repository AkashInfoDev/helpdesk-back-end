...
// const jwt = require("jsonwebtoken");
// const { User, Role, AdminUser } = require("../models");
...
module.exports = async (req, res, next) => {
    try {
        ...
        // Check if this is an admin token (has role_name directly)
        if (decoded.role_name === "admin") {
            // Admin user - attach directly from token
            req.user = {
                id: decoded.id,
                email: decoded.email,
                name: decoded.name || "Admin",
                role_id: 1, // Admin role_id
                role_name: "admin", // 🔥 CRITICAL
                status: "active",
            };
        ...
        // Regular user (customer/agent) - fetch from DB
        const user = await User.findByPk(decoded.id, {
            include: [{ model: Role, attributes: ["id", "name"] }],
        });
